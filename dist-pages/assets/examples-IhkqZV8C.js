import"./modulepreload-polyfill-B5Qt9EMX.js";import{s as w}from"./pinned-hero-top-PAvN5BGZ.js";import{c as M,o as r,t as e,d as k,p as v,e as S,g as T,h as C,f as q,a as s,S as f,b as O}from"./lib-D1jLtzyH.js";const y=document.querySelector("#app");if(!y)throw new Error("Cannot find #app root element");function g(){const t="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css";if(document.querySelector(`link[data-sod-icons="bootstrap"][href="${t}"]`))return;const o=document.createElement("link");o.rel="stylesheet",o.href=t,o.dataset.sodIcons="bootstrap",document.head.append(o)}y.innerHTML=`
...
`;const E=document.querySelector("#open-modal-minimal"),B=document.querySelector("#open-modal-basic"),L=document.querySelector("#open-modal-position"),A=document.querySelector("#open-modal-advanced"),$=document.querySelector("#open-confirm"),D=document.querySelector("#open-prompt"),I=document.querySelector("#open-form"),i=document.querySelector("#modal-result"),d=document.querySelector("#form-result"),R=document.querySelector("#run-adapter-open"),P=document.querySelector("#toggle-legacy-skin"),V=document.querySelector("#enable-adapter-context"),n=document.querySelector("#adapter-result");M({modalDefaults:{closeOnEsc:!0,closeOnBackdrop:!0,footerAlign:"center"},toastDefaults:{placement:"top-end",maxVisible:4,newestOnTop:!0,duplicateStrategy:"stack",duration:3800}});E?.addEventListener("click",()=>{r({title:"最小示例",content:"<p>这是最小可用的 Modal 调用。</p>"})});B?.addEventListener("click",()=>{r({title:"订单确认",content:"<p>这是一个基础 Modal 示例，支持自定义内容和按钮。</p>",animation:"zoom",confirmText:"确认",cancelText:"取消"})});L?.addEventListener("click",()=>{r({title:"顶部提示弹窗",content:"<p>演示 top 定位与关闭策略。</p>",position:"top",animation:"slide",useModal:!0,closeOnBackdrop:!0,closeOnEsc:!0,confirmText:"我知道了",cancelText:"关闭"})});A?.addEventListener("click",()=>{r({id:"advanced-modal-demo",title:"高级参数示例",content:'<div class="drag-anchor" style="margin-bottom:8px;font-weight:700;cursor:move;">拖拽锚点（也可拖动）</div><p>该示例演示 draggable / dragHandle / autoFitSize / scrollMode / 生命周期回调。</p><p>当内容较长时，会按 hybrid 规则切换滚动容器。</p>',animation:"fade",position:"center",draggable:!0,dragHandle:["header",".drag-anchor"],autoFitSize:!0,scrollMode:"hybrid",hybridSwitchRatio:1.25,confirmText:"确认",cancelText:"取消",onAfterOpen:()=>{e({title:"after-open",content:"高级 Modal 已打开",variant:"info",duration:1200})},onAfterClose:()=>{e({title:"after-close",content:"高级 Modal 已关闭",variant:"default",duration:1200})}})});let m=!1;R?.addEventListener("click",()=>{k({title:"Adapter Dialog",content:"<p>通过统一入口打开 Dialog，并带 traceId。</p>",traceId:"trace-example-001",onLayoutStable:({traceId:t})=>{v("success","布局稳定，可初始化第三方组件",{traceId:t,duration:1400}),n&&(n.textContent=`结果输出：onLayoutStable 已触发（traceId=${t??"-"})`)},onAction:({action:t,traceId:a})=>{n&&(n.textContent=`结果输出：action=${t}（traceId=${a??"-"})`)}})});P?.addEventListener("click",()=>{document.body.classList.toggle("legacy-skin");const t=document.body.classList.contains("legacy-skin");n&&(n.textContent=`结果输出：legacy-skin ${t?"已启用":"已关闭"}`),v("info",t?"已启用 legacy-skin 兼容主题":"已关闭 legacy-skin 兼容主题",{duration:1200,traceId:"trace-example-theme-001"})});V?.addEventListener("click",()=>{m||(g(),S({target:"#adapter-context-board .context-demo-row",traceId:"trace-example-ctx-001",items:[{id:"copy",label:"复制",icon:"bi bi-copy"},{id:"inspect",label:"查看",icon:"bi bi-eye"}],onAction:({itemId:t,triggerElement:a,traceId:o})=>{const h=a.dataset.fileName??"(unknown)";n&&(n.textContent=`结果输出：adapter-menu ${t} -> ${h}（traceId=${o??"-"})`)}}),m=!0),n&&(n.textContent="结果输出：适配层右键菜单已启用，请在卡片中文件行右键。")});$?.addEventListener("click",async()=>{const t=await T({title:"删除确认",content:"<p>确定删除这条记录吗？该操作不可恢复。</p>",confirmText:"删除",cancelText:"返回"});e({title:t?"已确认":"已取消",content:t?"用户确认执行删除。":"用户取消了删除。",variant:t?"danger":"info",duration:1800}),i&&(i.textContent=`结果输出：confirmModal => ${t?"true":"false"}`)});D?.addEventListener("click",async()=>{const t=await C({title:"请输入标签名",placeholder:"例如 release-note",validate:a=>a.length<2?"至少输入 2 个字符":!0});if(t===null){e({title:"已取消",content:"你取消了输入。",variant:"info",duration:1700}),i&&(i.textContent="结果输出：promptModal => null");return}e({title:"输入完成",content:`你输入的是: ${t}`,variant:"success",duration:2200}),i&&(i.textContent=`结果输出：promptModal => "${t}"`)});I?.addEventListener("click",async()=>{const t=await q({title:"创建发布计划",content:"<p>请填写发布信息，用于生成计划卡片。</p>",submitText:"创建",fields:[{name:"title",label:"计划标题",placeholder:"例如 v0.1.18 发布",required:!0,validate:a=>String(a??"").length<4?"标题至少 4 个字符":!0},{name:"owner",label:"负责人",placeholder:"例如 Alice",required:!0},{name:"priority",label:"优先级",type:"select",options:[{label:"P0 - 紧急",value:"p0"},{label:"P1 - 高",value:"p1"},{label:"P2 - 常规",value:"p2"}],defaultValue:"p1"},{name:"estimate",label:"预估工时(小时)",type:"number",defaultValue:6,required:!0},{name:"notes",label:"备注",type:"textarea",placeholder:"写下本次发布重点...",rows:3},{name:"notify",label:"发布后通知团队",type:"checkbox",defaultValue:!0}],validate:a=>{const o=a.estimate;return typeof o=="number"&&o>24?{estimate:"单次计划建议不超过 24 小时"}:!0}});if(t===null){e({title:"已取消",content:"你取消了表单提交。",variant:"info",duration:1600}),d&&(d.textContent="结果输出：formModal => null");return}e({title:"表单已提交",content:`标题: ${String(t.title??"")}`,variant:"success",duration:2200}),d&&(d.textContent=`结果输出：formModal => ${JSON.stringify(t)}`)});const z=document.querySelector("#open-offcanvas-minimal"),H=document.querySelector("#open-offcanvas-basic"),N=document.querySelector("#open-offcanvas-top"),j=document.querySelector("#open-offcanvas-bottom"),F=document.querySelector("#open-offcanvas-advanced");z?.addEventListener("click",()=>{s({title:"最小 Offcanvas",content:"<p>这是最小可用的 Offcanvas 调用。</p>"})});H?.addEventListener("click",()=>{s({title:"系统设置",placement:"end",animation:"slide",content:"<p>这里放筛选与配置项。</p>",confirmText:"保存",cancelText:"关闭"})});N?.addEventListener("click",()=>{s({title:"顶部通知面板",placement:"top",animation:"fade",content:"<p>适合展示全局通知或快捷操作。</p>",confirmText:"知道了",cancelText:"关闭"})});j?.addEventListener("click",()=>{s({title:"底部操作面板",placement:"bottom",animation:"zoom",content:"<p>适合移动端的底部操作入口。</p>",confirmText:"继续",cancelText:"取消"})});F?.addEventListener("click",()=>{s({id:"offcanvas-advanced-demo",title:"高级 Offcanvas",placement:"start",animation:"slide",draggable:!0,dragHandle:["header","body"],autoFitSize:!0,scrollMode:"hybrid",hybridSwitchRatio:1.2,closeOnBackdrop:!0,closeOnEsc:!0,content:"<p>左侧高级面板：支持拖拽、内容自适配和生命周期提示。</p>",confirmText:"保存",cancelText:"关闭",onAfterOpen:()=>{e({title:"offcanvas",content:"高级 Offcanvas 已打开",variant:"info",duration:1200})},onAfterClose:()=>{e({title:"offcanvas",content:"高级 Offcanvas 已关闭",variant:"default",duration:1200})}})});const J=document.querySelector("#open-toast-minimal"),G=document.querySelector("#open-toast-basic"),Q=document.querySelector("#open-toast-handle"),U=document.querySelector("#open-toast-queue"),W=document.querySelector("#open-toast-dup-update"),_=document.querySelector("#open-toast-dup-restart"),K=document.querySelector("#open-toast-dup-stack"),X=document.querySelector("#clear-toast"),Y=document.querySelector("#enable-context-menu"),Z=document.querySelector("#open-context-menu-manual"),tt=document.querySelector("#update-context-menu-items"),c=document.querySelector("#context-menu-result"),et=document.querySelector("#context-demo-board");let b=!1,l=!1,p=null;const x=()=>[{id:"copy",label:"复制文件名",icon:"bi bi-copy",onClick:({triggerElement:t})=>{const a=t.dataset.fileName??"(unknown)";navigator.clipboard.writeText(a)}},{id:"rename",label:"重命名",icon:"bi bi-pencil-square"},{id:"delete",label:"删除",icon:"bi bi-trash",iconAriaLabel:"Delete"}],at=()=>[{id:"pin",label:"置顶文件",icon:"bi bi-pin-angle"},{id:"share",label:"分享链接",icon:"bi bi-share"},{id:"inspect",label:"查看详情",icon:"bi bi-eye"}],u=()=>{b||(g(),p=O({target:".context-demo-row",items:x(),closeOnOutsideClick:!0,closeOnEsc:!0,closeOnWindowBlur:!0,closeOnScroll:!0,closeOnResize:!0,onAction:({itemId:t,triggerElement:a})=>{const o=a.dataset.fileName??"(unknown)";c&&(c.textContent=`结果输出：${t} -> ${o}`),e({title:"ContextMenu",content:`${t} -> ${o}`,variant:"info",duration:1200})},onClose:t=>{c&&(c.textContent=`结果输出：菜单已关闭（${t}）`)}}),b=!0)};J?.addEventListener("click",()=>{e({content:"最小通知示例"})});G?.addEventListener("click",()=>{e({title:"保存成功",content:"配置已同步",placement:"top-end",variant:"success",duration:2200,maxVisible:3,showProgress:!0})});Q?.addEventListener("click",()=>{const t=e({id:"manual-toast",title:"上传任务",content:"准备上传...",duration:!1,closable:!0,variant:"info"});window.setTimeout(()=>{t.update({content:"上传中 65%",variant:"warning"})},700),window.setTimeout(()=>{t.update({content:"上传完成",variant:"success",duration:1200}),t.resume()},1500)});U?.addEventListener("click",()=>{f.configure({maxVisible:3});for(let t=1;t<=6;t+=1)e({title:`队列消息 ${t}`,content:`第 ${t} 条示例提醒`,placement:"top-end",variant:"default",duration:1e3+t*250})});W?.addEventListener("click",()=>{e({id:"sync-job",title:"同步任务",content:"策略: update",variant:"info",duration:1800,duplicateStrategy:"update"})});_?.addEventListener("click",()=>{e({id:"sync-job",title:"同步任务",content:"策略: restart-timer",variant:"warning",duration:1800,duplicateStrategy:"restart-timer"})});K?.addEventListener("click",()=>{e({id:"sync-job",title:"同步任务",content:"策略: stack",variant:"success",duration:1800,duplicateStrategy:"stack"})});X?.addEventListener("click",()=>{f.closeAll()});Y?.addEventListener("click",()=>{u(),c&&(c.textContent="结果输出：右键任意文件行以触发菜单。"),e({title:"ContextMenu 已启用",content:"请在下方文件行点击右键。",variant:"success",duration:1500})});Z?.addEventListener("click",()=>{u();const t=et?.querySelector(".context-demo-row");p?.openAt(320,240,t??void 0),c&&(c.textContent="结果输出：已手动在 (320, 240) 打开菜单。")});tt?.addEventListener("click",()=>{u(),l=!l,p?.setItems(l?at():x()),c&&(c.textContent=`结果输出：菜单项已切换为${l?"备用组":"默认组"}。`),e({title:"ContextMenu",content:l?"已切换为备用菜单项":"已恢复默认菜单项",variant:"default",duration:1200})});const ot=document.querySelectorAll(".copy-btn");ot.forEach(t=>{t.addEventListener("click",async()=>{const a=t.dataset.copyTarget;if(!a)return;const o=document.querySelector(`#${a} code`);if(o)try{await navigator.clipboard.writeText(o.textContent??""),t.textContent="已复制",window.setTimeout(()=>{t.textContent="复制"},1200)}catch{t.textContent="复制失败",window.setTimeout(()=>{t.textContent="复制"},1200)}})});w({adjustSidebarOffset:!0});
