import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css                  */import{b as l,e as i,o as u,i as m}from"./lib-MV8x0ERr.js";import{r as p,w as b}from"./lab-shared-C3tuUAEJ.js";const d=document.querySelector("#app");if(!d)throw new Error("Cannot find #app root element");d.innerHTML=`
...
`;b();const a=document.querySelector("#cm-basic-result");l({target:"#cm-basic-zone",items:[{id:"copy",label:"复制"},{id:"rename",label:"重命名"}],onAction:({itemId:e})=>{a&&(a.textContent=`结果输出：执行 action=${e}`),i("success",`已执行 ${e}`,{duration:1100})}});l({target:"#cm-policy-zone",closeOnEsc:!0,closeOnScroll:!0,closeOnWindowBlur:!0,typeaheadResetMs:700,items:[{id:"download",label:"下载 Download"},{id:"rename",label:"重命名 Rename"},{id:"delete",label:"删除 Delete"}],onOpen:e=>{const t=document.querySelector("#cm-policy-focus-result");t&&(t.textContent="焦点定位：菜单已打开，使用方向键或字母进行定位。")},onFocusItem:({itemId:e,itemElement:t})=>{const o=document.querySelector("#cm-policy-focus-result"),n=(t.textContent??"").trim();o&&(o.textContent=`焦点定位：${e} (${n})`)},onTypeahead:({query:e,matched:t,itemId:o,itemElement:n})=>{const s=document.querySelector("#cm-policy-typeahead-result"),r=n?(n.textContent??"").trim():"-";s&&(s.textContent=t?`字母定位：query=${e} 命中 ${o??"-"} (${r})`:`字母定位：query=${e} 未命中`)},onAction:({itemId:e})=>{const t=document.querySelector("#cm-policy-action-result");t&&(t.textContent=`最近动作：${e}`),i("warning",`已触发动作: ${e}`,{duration:1200})},onClose:e=>{const t=document.querySelector("#cm-policy-close-result");t&&(t.textContent=`关闭原因：${e}`)}});document.querySelector("#cm-open-modal")?.addEventListener("click",()=>{u({title:"普通入口 Modal",content:"<p>用于与 ContextMenu 入口对比。</p>"})});let c=null;c=l({target:"#cm-dialog-zone",items:[{id:"open-dialog",label:"打开确认弹窗",onClick:()=>{c&&m(c,{title:"来自 ContextMenu",content:"<p>已执行 close-first-open-next。</p>"})}}]});
