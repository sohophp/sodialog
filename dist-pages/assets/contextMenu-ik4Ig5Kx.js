import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css                  */import{b as n,e as s,o as d,i as r}from"./lib-CaVldllS.js";import{r as u,w as m}from"./lab-shared-C3tuUAEJ.js";const i=document.querySelector("#app");if(!i)throw new Error("Cannot find #app root element");i.innerHTML=`
${u("context-menu","ContextMenu Lab","独立页面展示 ContextMenu 的基础绑定、关闭策略和菜单触发弹窗时序。")}
...
`;m();const c=document.querySelector("#cm-basic-result");n({target:"#cm-basic-zone",items:[{id:"copy",label:"复制"},{id:"rename",label:"重命名"}],onAction:({itemId:e})=>{c&&(c.textContent=`结果输出：执行 action=${e}`),s("success",`已执行 ${e}`,{duration:1100})}});n({target:"#cm-policy-zone",closeOnEsc:!0,closeOnScroll:!0,closeOnWindowBlur:!0,items:[{id:"download",label:"下载 Download"},{id:"rename",label:"重命名 Rename"},{id:"delete",label:"删除 Delete"}],onOpen:e=>{const o=document.querySelector("#cm-policy-focus-result");o&&(o.textContent="焦点定位：菜单已打开，使用方向键或字母进行定位。")},onFocusItem:({itemId:e,itemElement:o})=>{const l=document.querySelector("#cm-policy-focus-result"),a=(o.textContent??"").trim();l&&(l.textContent=`焦点定位：${e} (${a})`)},onAction:({itemId:e})=>{const o=document.querySelector("#cm-policy-action-result");o&&(o.textContent=`最近动作：${e}`),s("warning",`已触发动作: ${e}`,{duration:1200})},onClose:e=>{const o=document.querySelector("#cm-policy-close-result");o&&(o.textContent=`关闭原因：${e}`)}});document.querySelector("#cm-open-modal")?.addEventListener("click",()=>{d({title:"普通入口 Modal",content:"<p>用于与 ContextMenu 入口对比。</p>"})});let t=null;t=n({target:"#cm-dialog-zone",items:[{id:"open-dialog",label:"打开确认弹窗",onClick:()=>{t&&r(t,{title:"来自 ContextMenu",content:"<p>已执行 close-first-open-next。</p>"})}}]});
