import"./modulepreload-polyfill-B5Qt9EMX.js";function v(e={}){const o=e.heroSelector??".hero",a=e.topRowSelector??".hero-top",p=e.thresholdBase??56,m=e.adjustSidebarOffset??!1,n=e.offsetVarName??"--sod-sticky-offset",r=document.querySelector(o),t=document.querySelector(a);if(!r||!t)return;const i=document.documentElement.style;let s=!1;const c=()=>{s=!1;const f=Math.max(p,r.offsetTop+20),d=window.scrollY>f;if(t.classList.toggle("is-pinned",d),!m)return;if(!d){i.removeProperty(n);return}const u=Number.parseFloat(window.getComputedStyle(t).top)||0,g=Math.ceil(t.offsetHeight+u+14);i.setProperty(n,`${g}px`)},h=()=>{s||(s=!0,window.requestAnimationFrame(c))};c(),window.addEventListener("scroll",h,{passive:!0}),window.addEventListener("resize",c)}const l=document.querySelector("#app");if(!l)throw new Error("Cannot find #app root element");l.innerHTML=`
...
`;const b=document.querySelectorAll(".copy-btn");b.forEach(e=>{e.addEventListener("click",async()=>{const o=e.dataset.copyTarget;if(!o)return;const a=document.querySelector(`#${o} code`);if(a)try{await navigator.clipboard.writeText(a.textContent??""),e.textContent="已复制",window.setTimeout(()=>{e.textContent="复制"},1200)}catch{e.textContent="复制失败",window.setTimeout(()=>{e.textContent="复制"},1200)}})});v();
